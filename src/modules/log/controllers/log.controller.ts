...
export class LogController {
  ...
  async testCreate() {
    for (let i = 0; i <= 1000000; i++) {
      await this.logService.testCreate(this.generateData(i));
    }
    return;
  }
  ...
  async testBulkInsert() {
    let data: CreateLogDto[] = [];
    for (let i = 0; i <= 1000000; i++) {
      data.push(this.generateData(i));
    }
    await this.logService.testBulkInsert(data);
    return;
  }
  ...
  generateData(id: number): CreateLogDto {
    return {
      title: 'test',
      data: {
        id: id,
